...
export default function AppShell({ children, hideInputBar = false }: Props) {
  const { setSidebarOpen, toggleListening, isListening, isThinking, voiceStatus } = useVoice();
  const isProcessing = voiceStatus === "processing";
  const isSpeaking = voiceStatus === "speaking";
  const orbActive = isListening || isThinking || isSpeaking || isProcessing;
      ...
      <header className="flex items-center justify-between px-5 py-4 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded-xl transition-all active:scale-95 relative"
            style={{
              background: orbActive
                ? "linear-gradient(135deg, #4f8ef7, #7c5ef8)"
              ...
              boxShadow: orbActive
                ? "0 0 14px rgba(79,142,247,0.45)"
                : "0 2px 8px rgba(100,130,200,0.12)",
            }}
          >
            ...
            {orbActive && (
              <span
                className="absolute inset-0 rounded-xl animate-ring-pulse pointer-events-none"
