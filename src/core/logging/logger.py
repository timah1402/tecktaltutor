...
class Logger:
    ...
    def tool_call(
        self, tool_name: str, status: str = "success", elapsed_ms: float | None = None, **kwargs
    ):
        """
        Log tool call.
    ...
    def log_llm_output(
        self, agent_name: str, stage: str, response: str, metadata: dict[str, Any] | None = None
    ):
...
if __name__ == "__main__":
    ...
    solver = get_logger("Solver")
    research = get_logger("Research")
    guide = get_logger("Guide")

    # Standard logging
    solver.info("Initializing solver...")
    solver.progress("Loading knowledge base")
    solver.success("Ready", elapsed=2.3)

    # Stage logging
    research.stage("Analysis", "start")
    research.stage("Analysis", "running", "Processing 5 sources")
    research.stage("Analysis", "complete")

    # Tool calls
    guide.tool_call("RAG Search", status="success", elapsed_ms=150)
    guide.tool_call("Web Search", status="error", elapsed_ms=3000)

    # LLM calls
    solver.llm_call("gpt-4", agent="Planner", tokens_in=500, tokens_out=200, elapsed=1.5)

    # Warnings and errors
    research.warning("Rate limit approaching")
    guide.error("Failed to connect to database")

    print("\n" + "=" * 60)
    print("Demo complete! Logs saved to: user/logs/")
    print("=" * 60)
