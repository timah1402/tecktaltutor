    ...
    async def fetch_models(
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                is_likely_local_ollama = (binding == "ollama" or "ollama" in base_url.lower()) and "ollama.com" not in base_url.lower() or ":11434" in base_url
                ...
                async with session.get(url, headers=headers) as resp:
                    if resp.status == 200:
                        ...
                        if "data" in data and isinstance(data["data"], list):
                            return [m.get("id") or m.get("name") for m in data["data"] if m.get("id") or m.get("name")]
                        ...
                        # Handle raw list response
                            ...
                            if data and isinstance(data[0], dict):
                                return [m.get("id") or m.get("name") for m in data if m.get("id") or m.get("name")]
                            else:
...
async def llm_complete(
    ...
    **kwargs
    ...
    try:
        ...
        is_openai_compatible = binding.lower() in ["openai", "ollama", "ollama-cloud", "deepseek", "openrouter", "gemini"]
                    ...
                    async with session.post(url, headers=headers, json=data) as resp:
                        if resp.status == 200:
                            ...
                            if "choices" in result and result["choices"]:
                                ...
                                if not content:
                                    content = msg.get("reasoning_content") or msg.get("reasoning") or msg.get("thought") or ""
                                
                                if content:
                                    return content
                        else:
                            error_text = await resp.text()
                            print(f"❌ Fallback call failed with status={resp.status}: {error_text}")
