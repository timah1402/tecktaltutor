...
def generate_modern_roster_svg(
    ...
    display_users = list(reversed(users[-display_count:])) if len(users) >= display_count else list(reversed(users))
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        future_to_user = {
            executor.submit(fetch_avatar_as_base64, user["avatar_url"], avatar_size * 2): user["login"]
    ...
    for i, user in enumerate(display_users):
        ...
        clip_id = f"clip-{i}"
        svg_parts.append(f'<clipPath id="{clip_id}"><circle cx="{x + avatar_size/2}" cy="{avatar_y + avatar_size/2}" r="{avatar_size/2}"/></clipPath>')
...
def main():
    ...
    forkers = [{"login": f["owner"]["login"], "avatar_url": f["owner"]["avatar_url"]} for f in forks]
    ...
    stargazers_svg = generate_modern_roster_svg(
        stargazers, stargazers_total, "Stargazers",
        theme=args.theme, display_count=args.display
    )
